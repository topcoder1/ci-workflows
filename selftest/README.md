...
- `test_codex_cli_install.py` — executes `codex-review.yml`'s shipped
  ...
  binary the registry lists late is retried after 60/120/240 s with
  `--prefer-online` (the npm stub replays its cached packument without it,
  as npm 10.9.8 measurably does); a binary that never lands fails the step
  closed with an `::error::`. Negative controls: the pre-fix one-liner plus
  verify/retry/cache-bypass/backoff/fail-closed mutations.
  Background: on 2026-09-23 attaxion_dev#374's Codex job installed
  0.156.1 257 s after its linux-x64 binary was published, from a packument
  (max-age=300) that did not list it yet; npm skipped the optional
  dependency silently ("added 1 package") and the next step died on
