...
const archive = Buffer.concat([Buffer.from("504b0304", "hex"), Buffer.from("synthetic archive bytes; structural ZIP validation is separate")]);
const hash = (bytes) => createHash("sha256").update(bytes).digest("hex");
const selector = () => ({ runId: 101, runAttempt: 2, artifactId: 301 });
const producer = () => ({ repository: "example/reviewer", repositoryId: 11, workflowId: 21, workflowPath: ".github/workflows/review.yml", workflowRevision: revision, artifactName: "review.json" });
...
function run() {
  return { id: 101, workflow_id: 21, repository: { id: 11, full_name: "example/reviewer" }, head_repository: { id: 11, full_name: "example/reviewer" }, head_sha: revision, run_attempt: 2, status: "completed", conclusion: "success", event: "workflow_dispatch", path: ".github/workflows/review.yml", referenced_workflows: [], pull_requests: [], created_at: iso(now - 100000), updated_at: iso(now - 10000), run_started_at: iso(now - 50000) };
}
function artifact() {
  return { id: 301, name: "review.json", size_in_bytes: archive.length, digest: `sha256:${hash(archive)}`, expired: false, created_at: iso(now - 40000), updated_at: iso(now - 20000), expires_at: iso(now + 3600000), workflow_run: { id: 101, repository_id: 11, head_repository_id: 11, head_sha: revision } };
}
...
function fixture(options = {}) {
  ...
  const fetchImpl = async (url, init) => {
    ...
    if (url === zipURL) return options.direct ? new Response(archive) : new Response(null, { status: 302, headers: { location: signedURL } });
    if (url === signedURL) return new Response(archive);
    throw new Error(`Unexpected fake request ${url}`);
  };
  const client = createGitHubArtifactClient({ producer: options.producer ?? producer(), downloadOrigins: options.origins ?? [origin], fetchImpl, tokenProvider: async (request) => {
    credentialCalls++;
    assert.equal(request.repositoryId, 11);
    assert.equal(request.repository, "example/reviewer");
    assert.ok(Object.isFrozen(request));
    return options.tokenProvider ? options.tokenProvider(request) : "synthetic-token";
  } });
...
test("construction is inert; verified facts are frozen and archive access is defensive", async () => {
  ...
  assert.deepEqual(f.calls.map((call) => call.url), [runURL, attemptURL, artifactURL, zipURL, signedURL, runURL, artifactURL]);
  assert.ok(Object.isFrozen(result) && Object.isFrozen(result.producer) && Object.isFrozen(result.run) && Object.isFrozen(result.artifact));
  ...
  assert.deepEqual(result.archiveBytes, archive);
  for (const field of ["artifactAttemptAuthenticated", "executionComparisonAuthenticated", "enforcementPublished"]) assert.equal(result[field], false);
  for (const field of ["target", "metadata", "events", "ledger", "publisherActorId", "complete", "outcome", "policyDigest"]) assert.equal(Object.hasOwn(result, field), false);
  for (const { url, init } of f.calls) {
    ...
    if (url.startsWith(origin)) assert.deepEqual(init.headers, { accept: "application/octet-stream" });
...
test("direct API archive response and exact immutable path suffix are supported", async () => {
  const f = fixture({ direct: true, respond: ({ url }) => {
    if (url === runURL || url === attemptURL) return json({ ...run(), path: `${producer().workflowPath}@${revision}` });
  } });
...
test("configuration and selector are copied before asynchronous work", async () => {
  ...
  const f = fixture({ producer: p, origins, tokenProvider: async () => {
    input.runId = 999;
...
const runMutations = [
  ["run id", (r) => { r.id++; }, "run_binding_mismatch"],
  ["workflow id", (r) => { r.workflow_id++; }, "run_binding_mismatch"],
  ["repository id", (r) => { r.repository.id++; }, "run_binding_mismatch"],
  ["repository name", (r) => { r.repository.full_name = "other/repo"; }, "run_binding_mismatch"],
  ["fork id", (r) => { r.head_repository.id++; }, "run_binding_mismatch"],
  ["fork name", (r) => { r.head_repository.full_name = "other/repo"; }, "run_binding_mismatch"],
  ["approved source", (r) => { r.head_sha = "b".repeat(40); }, "run_binding_mismatch"],
  ["attempt", (r) => { r.run_attempt++; }, "run_not_current_success"],
  ["running", (r) => { r.status = "in_progress"; }, "run_not_current_success"],
  ["failed", (r) => { r.conclusion = "failure"; }, "run_not_current_success"],
  ["PR event", (r) => { r.event = "pull_request"; }, "unsupported_execution"],
  ["path", (r) => { r.path = ".github/workflows/other.yml"; }, "unsupported_execution"],
  ["mutable suffix", (r) => { r.path += "@main"; }, "unsupported_execution"],
  ["reusable workflow", (r) => { r.referenced_workflows = [{ sha: revision }]; }, "unsupported_execution"],
  ["missing reuse metadata", (r) => { delete r.referenced_workflows; }, "unsupported_execution"],
  ["PR association", (r) => { r.pull_requests = [{ number: 1 }]; }, "unsupported_execution"],
];
for (const [name, mutate, code] of runMutations) {
  test(`rejects mismatched ${name}`, async () => {
    const f = fixture({ respond: ({ url }) => {
      if (url === runURL) { const value = run(); mutate(value); return json(value); }
    } });
...
test("attempt endpoint must independently match authenticated current run", async () => {
  const f = fixture({ respond: ({ url }) => url === attemptURL ? json({ ...run(), updated_at: iso(now - 5000) }) : undefined });
  await fails(f.client, "metadata_changed");
});
const artifactMutations = [
  ["id", (a) => { a.id++; }, "artifact_binding_mismatch"],
  ["name", (a) => { a.name = "other.json"; }, "artifact_binding_mismatch"],
  ["run", (a) => { a.workflow_run.id++; }, "artifact_binding_mismatch"],
  ["repo", (a) => { a.workflow_run.repository_id++; }, "artifact_binding_mismatch"],
  ["head repo", (a) => { a.workflow_run.head_repository_id++; }, "artifact_binding_mismatch"],
  ["head SHA", (a) => { a.workflow_run.head_sha = "b".repeat(40); }, "artifact_binding_mismatch"],
  ["expired flag", (a) => { a.expired = true; }, "artifact_expired"],
  ["expired date", (a) => { a.expires_at = iso(now - 1000); }, "artifact_expired"],
  ["missing digest", (a) => { delete a.digest; }, "archive_digest_missing"],
  ["wrong digest algorithm", (a) => { a.digest = `md5:${"a".repeat(64)}`; }, "archive_digest_missing"],
  ["oversize", (a) => { a.size_in_bytes = GITHUB_ARTIFACT_LIMITS.archiveBytes + 1; }, "archive_limit"],
  ["zero size", (a) => { a.size_in_bytes = 0; }, "archive_limit"],
];
for (const [name, mutate, code] of artifactMutations) {
  test(`rejects artifact ${name}`, async () => {
    const f = fixture({ respond: ({ url }) => {
      if (url === artifactURL) { const value = artifact(); mutate(value); return json(value); }
    } });
    await fails(f.client, code);
...
test("changed bytes fail the authenticated archive digest", async () => {
  const changed = Buffer.from(archive); changed[10] ^= 1;
  const f = fixture({ respond: ({ url }) => url === signedURL ? new Response(changed) : undefined });
  await fails(f.client, "archive_digest_mismatch");
});
test("archive length must match authenticated size", async () => {
  const f = fixture({ respond: ({ url }) => url === signedURL ? new Response(archive.subarray(1)) : undefined });
...
test("raw upload-artifact output is refused even with a valid authenticated digest", async () => {
  ...
  const f = fixture({ respond: ({ url }) => {
    if (url === artifactURL) return json({ ...artifact(), size_in_bytes: raw.length, digest: `sha256:${hash(raw)}` });
...
test("run association never authenticates the artifact attempt", async () => {
  const f = fixture({ respond: ({ url }) => url === artifactURL ? json({ ...artifact(), created_at: iso(now - 90000) }) : undefined });
  const result = await f.client.read(selector());
...
test("rerun beginning during download is rejected", async () => {
  const f = fixture({ respond: ({ url, count }) => url === runURL && count === 2 ? json({ ...run(), run_attempt: 3, status: "queued" }) : undefined });
...
for (const change of ["deleted", "expired", "digest", "updated", "binding"]) {
  test(`fresh artifact check rejects ${change}`, async () => {
    const f = fixture({ respond: ({ url, count }) => {
      if (url !== artifactURL || count !== 2) return;
      if (change === "deleted") return new Response("not surfaced", { status: 404 });
      const a = artifact();
    ...
    await fails(f.client, { deleted: "metadata_http_error", expired: "artifact_expired", digest: "metadata_changed", updated: "metadata_changed", binding: "artifact_binding_mismatch" }[change]);
  });
}
for (const [location, code] of [
  ["https://artifacts.example.test.attacker.test/x?secret=signed", "untrusted_download_origin"],
  ...
  test(`rejects forbidden storage location (${code}, ${location.length})`, async () => {
    const f = fixture({ respond: ({ url }) => url === zipURL ? new Response(null, { status: 302, headers: { location } }) : undefined });
    await fails(f.client, code);
    assert.equal(f.calls.length, 4);
  });
}
test("second storage redirect is refused without forwarding sensitive headers", async () => {
  const f = fixture({ respond: ({ url }) => url === signedURL ? new Response(null, { status: 302, headers: { location: "https://other.example.test/?credential=secret" } }) : undefined });
  await fails(f.client, "archive_http_error");
  assert.equal(f.calls.length, 5);
  assert.deepEqual(f.calls[4].init.headers, { accept: "application/octet-stream" });
});
test("metadata redirect is refused, including implicit redirect by a broken fetch adapter", async () => {
  await fails(fixture({ respond: ({ url }) => url === runURL ? new Response(null, { status: 302, headers: { location: signedURL } }) : undefined }).client, "metadata_http_error");
  const response = json(run());
  Object.defineProperty(response, "redirected", { value: true });
  await fails(fixture({ respond: () => response }).client, "unexpected_redirect");
});
test("metadata body cap is enforced during streaming without content-length", async () => {
  let cancelled = false;
  const body = new ReadableStream({ start(c) { c.enqueue(Buffer.alloc(GITHUB_ARTIFACT_LIMITS.metadataBytes)); c.enqueue(Buffer.from("x")); }, cancel() { cancelled = true; } });
  await fails(fixture({ respond: ({ url }) => url === runURL ? new Response(body) : undefined }).client, "response_limit");
  assert.equal(cancelled, true);
});
test("archive streaming cap cancels the reader", async () => {
  let cancelled = false;
  const body = new ReadableStream({ start(c) { c.enqueue(Buffer.alloc(GITHUB_ARTIFACT_LIMITS.archiveBytes)); c.enqueue(Buffer.from("x")); }, cancel() { cancelled = true; } });
  await fails(fixture({ respond: ({ url }) => url === signedURL ? new Response(body) : undefined }).client, "response_limit");
  assert.equal(cancelled, true);
});
test("oversized content-length is rejected without consuming the body", async () => {
  let cancelled = false;
  const body = new ReadableStream({ cancel() { cancelled = true; } });
  await fails(fixture({ respond: ({ url }) => url === runURL ? new Response(body, { headers: { "content-length": String(GITHUB_ARTIFACT_LIMITS.metadataBytes + 1) } }) : undefined }).client, "response_limit");
  assert.equal(cancelled, true);
});
...
test("deadline spans all requests rather than restarting per response", async () => {
  const f = fixture({ respond: async () => { await new Promise((resolve) => setTimeout(resolve, 15)); } });
...
test("deadline interrupts slow body even if fetch does not implement abort", async () => {
  let cancelled = false;
  const f = fixture({ respond: ({ url }) => url === signedURL ? new Response(new ReadableStream({ cancel() { cancelled = true; } })) : undefined });
...
test("credential timeout never starts requests, even after provider eventually resolves", async () => {
  let finish;
  const f = fixture({ tokenProvider: () => new Promise((resolve) => { finish = resolve; }) });
...
test("a fetch settling after the shared deadline has its response body cancelled", async () => {
  let finish;
  let cancelled = false;
  const f = fixture({ respond: () => new Promise((resolve) => { finish = resolve; }) });
  await fails(f.client, "deadline_exceeded", selector(), { deadlineMs: 10 });
...
test("credential and transport exceptions cannot surface sensitive strings", async () => {
  await fails(fixture({ tokenProvider: () => { throw new Error("private-token"); } }).client, "credential_unavailable");
  await fails(fixture({ tokenProvider: () => "bad\r\ncredential" }).client, "credential_unavailable");
  await fails(fixture({ respond: () => { throw new Error("private signed URL or response body"); } }).client, "request_failed");
});
...
test("producer and origin input accessors are not invoked", () => {
  let invoked = false;
  const p = producer();
  Object.defineProperty(p, "repository", { enumerable: true, get() { invoked = true; return "other/repo"; } });
  assert.throws(() => fixture({ producer: p }), { code: "invalid_input" });
  const origins = [];
  Object.defineProperty(origins, "0", { enumerable: true, get() { invoked = true; return origin; } });
