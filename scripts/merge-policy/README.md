...
The [data-only producer](DATA-ONLY-PRODUCER.md) measures exact local Git objects, sends their complete bounded text to one structured reviewer call and emits the existing receipt format. It requires explicit successful review completion and preserves all reported findings. The dispatch identity is still a trusted caller input; no live workflow, authenticated artifact adapter, intake persistence or acceptance publication is wired to this producer.

`node --test selftest/test_merge_policy_*.mjs` runs the policy suites, including real local Git fixtures and mocked provider/API responses. These tests do not substitute for authenticated live review producers, App-source enforcement and live merge tests on GitHub.
