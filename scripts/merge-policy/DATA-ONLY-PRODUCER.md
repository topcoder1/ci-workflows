...
2. `collectReviewComparison({repositoryPath, baseSha, headSha}, {signal, deadlineMs})` measures the actual SHA-1 commits, unique merge base, changed-file inventory and complete before/after blob text. The base must be an ancestor of the head. Empty comparisons stop before a provider request.
   A trusted caller may also supply `expectedComparisonSha256` to `produce`. The producer snapshots and validates this optional lowercase SHA256, then requires the measured comparison to match before acquiring credentials or invoking the provider. Omission preserves existing behavior. This bounds the approved review content; it does not authenticate the caller, repository or execution.
...
5. Build the existing receipt with every validated finding and the snapshotted dispatch context. Return frozen data, defensive receipt bytes, SHA256 digests of the receipt, comparison, provider input/output and request, and the provider's reported token usage (the two validated integers a cost is computed from). No receipt is returned if the final canonical bytes exceed 64 KiB.
...
Initial limits are 32 changed files, 64 KiB per blob, 256 KiB of aggregate before/after text and a 512 KiB measured packet. Duplicate content appearing in multiple file snapshots still counts toward the aggregate limit. There is no truncation or partial-review fallback. Git subprocesses share a monotonic deadline and output caps; cancellation stops their process groups. The producer's own deadline spans Git measurement and the provider call. Review text bounds are 1,024 characters per finding title and 6,000 characters per reason and per summary; the prompt asks the model for under 160 and 1,500 respectively. The earlier 256/3,000 caps rejected roughly one in three otherwise valid live reviews on 2026-09-16, because structured-output schemas cannot carry string length constraints. Review intake and the ledger engine accept the same bounds; ledger event reasons, which prefix a finding key or the receipt commitment, are capped at 8,192 characters.
...
Failures retain closed diagnostic codes across the reviewer and producer, including credential, transport, HTTP, JSON/schema, incomplete-review, comparison, deadline and receipt failures. Unknown comparison errors remain `comparison_failed`; arbitrary thrown values cannot provide their own trusted codes. `anthropicReviewFailure` and `reviewProducerFailure` return only frozen metadata for errors created by their own module; they do not inspect exception getters, prototypes, message or stack. Provider metadata contains the code, a bounded numeric HTTP status when observed and `providerOutcome`; producer metadata also contains the phase. These diagnostics contain no original provider error or response body.

`providerOutcome` distinguishes `not_requested`, `unknown`, `response_received` and `review_completed`. Transport invocation alone leaves the provider outcome unknown. Receiving headers is not proof of complete review. `review_completed` can appear on a later receipt failure and still yields no receipt or acceptance. No automatic retry or deadline increase is introduced. The inactive probe persists these diagnostics separately after its private reservation and retains job failure when uploading evidence.
...
The inactive [fixed review execution package](staging/REVIEW-EXECUTION-PROBE.md) supplies a native entrypoint and pinned workflow template for one historical synthetic comparison. Its final report retains the exact receipt and provider digests but explicitly carries no current PR, execution or merge authority. It is not connected to intake and has not been dispatched by this source change.

The inactive [current-target review dispatch package](staging/REVIEW-DISPATCH.md) generalizes it: the target comes from validated `workflow_dispatch` inputs, the receipt is written and uploaded as exact wire bytes under the name a producers document would register, and a separate report carries the observations. It, too, is not connected to intake, registers no producer, and has not been dispatched.
