...
function inputRecord(input, fields, optional = []) {
  requireThat(
    input && [Object.prototype, null].includes(Object.getPrototypeOf(input)),
    "invalid_input",
  );
...
function utf8(bytes, code) {
  try {
    ...
    throw new ComparisonError(code);
  }
}
...
function operationScope(signal, milliseconds) {
  ...
  return {
    ...
    count(bytes) {
      byteCount += bytes;
      if (byteCount > REVIEW_COMPARISON_LIMITS.subprocessBytes)
      ...
      new Promise((resolve, reject) => {
        ...
        child.stdout.on("data", (chunk) => {
          ...
          try {
            ...
            fail(
              error instanceof ComparisonError ? error.code : "git_io_failed",
            );
          }
        });
        child.stderr.on("data", (chunk) => {
          ...
          try {
            ...
            requireThat(
              stderrBytes <= REVIEW_COMPARISON_LIMITS.stderrBytes,
        ...
        child.on("close", (code) => {
          ...
          resolve({
            bytes: Buffer.concat(chunks, stdoutBytes),
            exitCode: code,
          });
        });
...
function inventory(bytes) {
  ...
  for (let index = 0; index < bytes.length; index++) {
    ...
    start = index + 1;
    requireThat(
      fields.length <= REVIEW_COMPARISON_LIMITS.files * 2,
      "file_limit",
    );
  }
  ...
  for (let index = 0; index < fields.length; index += 2) {
    const match =
      /^:([0-7]{6}) ([0-7]{6}) ([a-f0-9]{40}) ([a-f0-9]{40}) ([AMDT])$/.exec(
        fields[index].toString("ascii"),
      );
    requireThat(match, "invalid_inventory");
    const [, beforeMode, afterMode, beforeOid, afterOid, status] = match;
    requireThat(
      ["000000", "100644", "100755"].includes(beforeMode) &&
        ["000000", "100644", "100755"].includes(afterMode),
      "unsupported_file_mode",
    );
    requireThat(
      (beforeMode === "000000") === (beforeOid === ZERO) &&
        (afterMode === "000000") === (afterOid === ZERO),
      "invalid_inventory",
    );
    requireThat(
      (status === "A" && beforeOid === ZERO && afterOid !== ZERO) ||
        (status === "D" && beforeOid !== ZERO && afterOid === ZERO) ||
        (status === "M" && beforeOid !== ZERO && afterOid !== ZERO),
      "invalid_inventory",
    );
    const pathBytes = fields[index + 1];
    requireThat(
      pathBytes.length > 0 &&
        pathBytes.length <= REVIEW_COMPARISON_LIMITS.pathBytes,
      "unsupported_path",
    );
...
function blobSizes(bytes, ids) {
  ...
  requireThat(
    lines.pop() === "" && lines.length === ids.length,
    "invalid_git_objects",
  );
  const sizes = new Map();
  for (let index = 0; index < ids.length; index++) {
    ...
    requireThat(
      Number.isSafeInteger(size) && size <= REVIEW_COMPARISON_LIMITS.blobBytes,
      "blob_limit",
    );
...
export async function collectReviewComparison(input, options = {}) {
  let scope;
  try {
    const { repositoryPath, baseSha, headSha } = inputRecord(input, [
      "repositoryPath",
      "baseSha",
      "headSha",
    ]);
    const { signal, deadlineMs = REVIEW_COMPARISON_LIMITS.deadlineMs } =
      inputRecord(options, [], ["signal", "deadlineMs"]);
    requireThat(
      typeof repositoryPath === "string" &&
        isAbsolute(repositoryPath) &&
        repositoryPath.length <= 4096 &&
        !CONTROL.test(repositoryPath),
      "invalid_input",
    );
    requireThat(
    ...
    const commitsResult = await git(
      root,
      ["cat-file", "--batch"],
      scope,
      commitIds.length * (REVIEW_COMPARISON_LIMITS.commitBytes + 128),
      `${commitIds.join("\n")}\n`,
    );
    const commits = parseBatch(
      commitsResult.bytes,
      commitIds,
      "commit",
      REVIEW_COMPARISON_LIMITS.commitBytes,
    );
    function tree(id) {
      const match = /^tree ([a-f0-9]{40})\n/.exec(
        commits.get(id).toString("ascii"),
      );
    ...
    const changed = await git(
      ...
      scope,
      REVIEW_COMPARISON_LIMITS.inventoryBytes,
    );
    const entries = inventory(changed.bytes);
    const blobIds = [
      ...new Set(
        entries
          .flatMap((entry) => [entry.beforeOid, entry.afterOid])
          .filter((oid) => oid !== ZERO),
      ),
    ];
    let sizes = new Map();
    ...
    if (blobIds.length) {
      ...
      totalContentBytes = entries.reduce(
        (sum, entry) =>
          sum +
          (sizes.get(entry.beforeOid) ?? 0) +
          (sizes.get(entry.afterOid) ?? 0),
        0,
      );
      requireThat(
        totalContentBytes <= REVIEW_COMPARISON_LIMITS.contentBytes,
        "content_limit",
      );
      const uniqueBytes = [...sizes.values()].reduce(
        (sum, size) => sum + size,
        0,
      );
      const objects = await git(
        ...
        ["cat-file", "--batch"],
        scope,
        uniqueBytes + blobIds.length * 128,
        `${blobIds.join("\n")}\n`,
      );
      blobs = parseBatch(
        objects.bytes,
        blobIds,
        "blob",
        REVIEW_COMPARISON_LIMITS.blobBytes,
      );
    }
    ...
    requireThat(
      bytes.length <= REVIEW_COMPARISON_LIMITS.packetBytes,
      "packet_limit",
    );
    scope.check();
    return Object.freeze({
