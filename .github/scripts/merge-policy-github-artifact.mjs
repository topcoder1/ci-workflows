...
function producerCopy(value) {
  ...
  requireThat(
    typeof p.repository === "string" &&
      ...
      ![".", ".."].includes(p.repository.split("/")[1]),
    "invalid_input",
  );
  requireThat(
    positive(p.repositoryId) && positive(p.workflowId),
    "invalid_input",
  );
  requireThat(
    typeof p.workflowPath === "string" &&
      /^\.github\/workflows\/[A-Za-z0-9_-]+\.ya?ml$/.test(p.workflowPath),
    "invalid_input",
  );
  requireThat(
    typeof p.workflowRevision === "string" &&
      /^[a-f0-9]{40}$/.test(p.workflowRevision),
    "invalid_input",
  );
  requireThat(
    typeof p.artifactName === "string" &&
...
function storageURL(value) {
  requireThat(
    typeof value === "string" && value.length <= 8192 && !/[\s\\#]/.test(value),
  ...
  try {
    ...
    throw new ArtifactError("invalid_download_url");
  }
...
function runFacts(raw, producer, selector) {
  requireThat(
    raw?.id === selector.runId &&
      raw.workflow_id === producer.workflowId &&
      raw.repository?.id === producer.repositoryId &&
      raw.repository?.full_name === producer.repository &&
      raw.head_repository?.id === producer.repositoryId &&
      raw.head_repository?.full_name === producer.repository &&
      raw.head_sha === producer.workflowRevision,
    "run_binding_mismatch",
  );
  requireThat(
    raw.run_attempt === selector.runAttempt &&
...
function artifactFacts(raw, producer, selector, run) {
  requireThat(
    raw?.id === selector.artifactId &&
      ...
      raw.workflow_run?.id === selector.runId &&
      raw.workflow_run.repository_id === producer.repositoryId &&
      raw.workflow_run.head_repository_id === producer.repositoryId &&
      raw.workflow_run.head_sha === producer.workflowRevision,
  ...
  requireThat(
    positive(raw.size_in_bytes) &&
      raw.size_in_bytes <= GITHUB_ARTIFACT_LIMITS.archiveBytes,
    "archive_limit",
  );
  requireThat(
    typeof raw.digest === "string" && /^sha256:[a-f0-9]{64}$/.test(raw.digest),
    "archive_digest_missing",
  );
  const createdAt = date(raw.created_at);
  const updatedAt = date(raw.updated_at);
  requireThat(
    Date.parse(createdAt) >= Date.parse(run.createdAt) &&
...
async function bytes(response, maximum, scope) {
  ...
  try {
    ...
    requireThat(
      total > 0 && (decoded || length === null || Number(length) === total),
