...
// so the key carries the validator's own pattern: a bare string was a likely
...
function array(value, maximum, code = "invalid_input") {
  ...
  requireThat(
    length <= maximum && Reflect.ownKeys(value).length === length + 1,
    code,
  );
...
function comparisonCopy(input) {
  ...
  requireThat(
    input.schemaVersion === 1 &&
      input.comparisonKind === "git-ancestor-text-v1" &&
      [
        input.baseSha,
        input.headSha,
        input.mergeBaseSha,
        input.baseTreeOid,
        input.headTreeOid,
      ].every((sha) => typeof sha === "string" && SHA.test(sha)) &&
      input.baseSha === input.mergeBaseSha &&
      input.baseSha !== input.headSha,
  );
  requireThat(
    input.githubIdentityAuthenticated === false &&
      input.reviewCompleted === false &&
      input.enforcementPublished === false,
  );
  requireThat(
    typeof input.comparisonSha256 === "string" &&
      DIGEST.test(input.comparisonSha256),
  );
  ...
  function blob(value) {
    ...
    requireThat(
      typeof value.oid === "string" &&
        SHA.test(value.oid) &&
        ["100644", "100755"].includes(value.mode) &&
    ...
    requireThat(
      typeof value.text === "string" &&
        ...
        value.text.length <= ANTHROPIC_REVIEW_LIMITS.blobBytes,
    );
    ...
    const oid = createHash("sha1")
      ...
      .digest("hex");
    requireThat(
      bytes.length <= ANTHROPIC_REVIEW_LIMITS.blobBytes &&
        total <= ANTHROPIC_REVIEW_LIMITS.contentBytes &&
  ...
  const files = input.files.map((file) => {
    ...
    requireThat(
      (file.status === "A" && before === null && after !== null) ||
        (file.status === "D" && before !== null && after === null) ||
        (file.status === "M" && before !== null && after !== null),
    );
    return { path: file.path, status: file.status, before, after };
  });
  requireThat(
    total === input.totalContentBytes &&
      files.every(
        (file, index) =>
          index === 0 ||
          Buffer.compare(
  ...
  requireThat(
    sha256(JSON.stringify(measured)) === input.comparisonSha256,
    "comparison_mismatch",
  );
  return freeze({
    ...measured,
  ...
  function value(depth = 0) {
    ...
    if (character === "{" || character === "[") {
      ...
      while (index < source.length) {
        ...
        if (source[index] === close) {
          index++;
          return object ? Object.fromEntries(entries) : entries;
        }
        if (source[index++] !== ",") return bad();
      }
      return bad();
...
function reviewResult(message, comparison) {
  ...
  requireThat(
    typeof message.id === "string" &&
      /^msg_[A-Za-z0-9_-]{1,128}$/.test(message.id) &&
      message.type === "message" &&
      message.role === "assistant" &&
      message.model === ANTHROPIC_REVIEW_MODEL,
    "response_identity_mismatch",
  );
  requireThat(
    message.stop_reason === "end_turn" &&
      message.stop_sequence === null &&
      (message.stop_details === undefined || message.stop_details === null) &&
      (message.container === undefined || message.container === null),
    "incomplete_review",
  );
  requireThat(
    message.usage !== null &&
      ...
      message.usage.output_tokens <= ANTHROPIC_REVIEW_LIMITS.maxTokens,
    "invalid_response",
  );
  ...
  requireThat(
    Buffer.byteLength(block.text) <= ANTHROPIC_REVIEW_LIMITS.outputBytes,
    "output_limit",
  );
  const review = parseJSON(block.text);
  shape(
    review,
    ["complete", "outcome", "findingCount", "summary", "findings"],
    [],
    "invalid_review",
  );
  ...
  requireThat(
    Number.isSafeInteger(review.findingCount) &&
      ...
      ((review.outcome === "clean" && review.findingCount === 0) ||
        (review.outcome === "findings" && review.findingCount > 0)),
    "incomplete_findings",
  );
  const keys = new Set();
...
async function readResponse(response, scope) {
  ...
  try {
    ...
    const coding = response.headers
      .get("content-encoding")
      ?.trim()
      .toLowerCase();
    requireThat(
      coding === undefined ||
        ["identity", "gzip", "deflate", "br"].includes(coding),
      "invalid_response",
    );
    ...
    requireThat(
      declared === null ||
        (/^[0-9]+$/.test(declared) &&
          Number(declared) <= ANTHROPIC_REVIEW_LIMITS.responseBytes),
      "response_limit",
    );
    ...
    while (true) {
      ...
      const chunk = chunkCopy(
        value,
        ANTHROPIC_REVIEW_LIMITS.responseBytes - total,
      );
      total += chunk.length;
      chunks.push(chunk);
    }
...
export function createAnthropicReviewer(options) {
  ...
  try {
    ...
    fetchImpl =
      options.fetchImpl === undefined ? globalThis.fetch : options.fetchImpl;
    requireThat(
      ...
      try {
        ...
        const deadlineMs =
          options.deadlineMs === undefined
            ? ANTHROPIC_REVIEW_LIMITS.deadlineMs
            : options.deadlineMs;
        requireThat(
          Number.isSafeInteger(deadlineMs) &&
            deadlineMs > 0 &&
            deadlineMs <= ANTHROPIC_REVIEW_LIMITS.deadlineMs &&
            (signal === undefined ||
        ...
        const body = JSON.stringify({
          ...
          max_tokens: ANTHROPIC_REVIEW_LIMITS.maxTokens,
          stream: false,
          system: SYSTEM,
          messages: [
        ...
        requireThat(
          Buffer.byteLength(body) <= ANTHROPIC_REVIEW_LIMITS.requestBytes,
          "request_limit",
        );
        scope.check();
        try {
          token = await scope.wait(() =>
            tokenProvider(Object.freeze({ signal: scope.signal })),
          );
        } catch {
          scope.check();
          throw new ReviewError("credential_unavailable");
        }
        requireThat(
          typeof token === "string" &&
            token.length > 0 &&
